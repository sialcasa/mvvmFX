...
public class MVVMViewLoader {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(MVVMViewLoader.class);

    /**
     * Gets a {@link MVVMView} and a {@link MVVMViewModel} for a given {@link ViewController}.
     * 
     * @param view to lookup
     * @return tuple
     */
    ...
    /**
     * Helper for loading a Controler from Fs.
     * 
    ...
    private MVVMTuple loadController(final String resource) {
        ...
        if (location == null) {
            LOG.error("Error loading FXML - can't load from given resourcepath: " + resource);
            return null;
        }
        ...
        final MVVMTuple controllerTuple = new MVVMTuple((MVVMView<?>) fxmlLoader.getController(), view);
        return controllerTuple;
    }
}
