 ...
 * 
 */
public final class UFXBindings {
     ...
     * 
     * @param fromPane {@link Pane}
     * @param toPane {@link Pane}
     */
    ...
    public static void bindSize(final Node from, final Pane toPane) {
        ...
        if (from instanceof Rectangle) {
            ...
            toPane.minHeightProperty().bind(((Rectangle) from).heightProperty());
            toPane.maxHeightProperty().bind(((Rectangle) from).heightProperty());
            return;
        }

        if (from instanceof ImageView) {
            toPane.minWidthProperty().bind(((ImageView) from).fitWidthProperty());
            toPane.maxWidthProperty().bind(((ImageView) from).fitWidthProperty());
            toPane.minHeightProperty().bind(((ImageView) from).fitHeightProperty());
            toPane.maxHeightProperty().bind(((ImageView) from).fitHeightProperty());
            return;
        }

        throw new IllegalArgumentException("Type of parameter is not mapped yet, please add mapping for type: "
                + from.getClass());
    }
    ...
    public static void bindWidth(final Node from, final Control toControl) {
        ...
        if (from instanceof Rectangle) {
            toControl.minWidthProperty().bind(((Rectangle) from).widthProperty());
            toControl.maxWidthProperty().bind(((Rectangle) from).widthProperty());
            return;
        }
    }

    /**
     * Utility to check, whether a node has a Parent.
     * 
     * @param node to check whether it has a parent
     * @return {@link Boolean}
     */
    public static boolean hasParent(final Node node) {
