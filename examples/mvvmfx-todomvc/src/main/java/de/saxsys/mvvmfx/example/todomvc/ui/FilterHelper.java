...
public class FilterHelper {
	...
	public static <T> ObservableList<T> filter(ObservableList<T> items, Function<T, ObservableBooleanValue> conditionExtractor) {
		return filterInternal(items, conditionExtractor, t -> conditionExtractor.apply(t).get());
	}

	public static <T> ObservableList<T> filterInverted(ObservableList<T> items, Function<T, ObservableBooleanValue> conditionExtractor) {
		return filterInternal(items, conditionExtractor, t -> !conditionExtractor.apply(t).get());
	}


	private static <T> ObservableList<T> filterInternal(ObservableList<T> items, Function<T, ObservableBooleanValue> conditionExtractor, final Predicate<T> predicate ) {
