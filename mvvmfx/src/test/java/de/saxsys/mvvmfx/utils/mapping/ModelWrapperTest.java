...
public class ModelWrapperTest {
	...
	public void testWithGetterAndSetter() {
		...
		phoneNumbersProperty.setValue(FXCollections.observableMap(Collections.singletonMap("private",
				"0351 7654321")));
	...
	public void testDifferentFlag() {
		...
		Map<String, String> phoneNumberMapHorst = new HashMap<>();
		phoneNumberMapHorst.put("private", "0351 1234567");
		person.setPhoneNumbers(FXCollections.observableMap(phoneNumberMapHorst));
		...
		phoneNumbers.remove("private", "0351 1234567");

		assertThat(personWrapper.isDifferent()).isTrue();

		emailAddresses.add("test@example.org");
		phoneNumbers.put("private", "0351 1234567");
		assertThat(personWrapper.isDifferent()).isFalse();

		emailAddresses.add("horst@example.org");
		phoneNumbers.put("privateNeu", "0351 123456789");
		assertThat(personWrapper.isDifferent()).isTrue();

		emailAddresses.remove("horst@example.org");
		phoneNumbers.remove("privateNeu", "0351 123456789");
		assertThat(personWrapper.isDifferent()).isFalse();

		emailAddresses.setValue(FXCollections.observableSet("horst@example.org"));
		phoneNumbers.setValue(FXCollections.observableMap(phoneNumberMapHorst));
		assertThat(personWrapper.isDifferent()).isTrue();
	...
	public void testDifferentFlagWithFxProperties() {
		...
		assertThat(personWrapper.isDifferent()).isTrue(); // still true because the modelWrapper can't detect the
		// change in the model
	...
	public void valuesShouldBeUpdatedWhenModelInstanceChanges() {
		...
		final SetProperty<String> emailAddresses = personWrapper.field(Person::getEmailAddresses,
				Person::setEmailAddresses, person1.getEmailAddresses());
		final MapProperty<String, String> phoneNumbers = personWrapper.field(Person::getPhoneNumbers,
