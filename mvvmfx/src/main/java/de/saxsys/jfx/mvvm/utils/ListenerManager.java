...
/**
 * The {@link ListenerManager} is used to be able to clean up listeners
 * when they are not used anymore. A typical use case is a component that
 * registers some listeners to its properties. When the component isn't used
 * anymore the listeners needs to be removed from the properties. Otherwise it could lead to memory leaks
 * as the garbage collector can't delete the objects as there are still references available.
 * 
 * With the {@link ListenerManager} you can reduce the possibility to forget a listener because all
 * listeners can be removed with one single method {@link ListenerManager#clean()}.
 * 
 * @author manuel.mauky
 * 
 */
public class ListenerManager implements ICleanable {
    ...
    /**
     * Register the given {@link ChangeListener} to the {@link ObservableValue}. The listener is added to the
    ...
    /**
     * This method is used to clear the given map. To do this you need to implement a BiConsumer that calls the
     * specific method to remove a listener from an observable.
     * 
     ...
     * function.
     * 
     * @param map the multimap that contains the observables and listeners.
     * @param consumer a function that calls the specific remove method for the given types.
     */
